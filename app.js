...
function generateHMS() {
    ...
    var colorCardsH = document.getElementsByClassName("color-hex");
  var rgbCodesH = document.getElementsByClassName("color-rgb");

  for (var i = 0; i < colorCardsH.length; i++) {
    colorCardsH[i].addEventListener('click', copyAttr, false);
    rgbCodesH[i].addEventListener('click', copyAttr, false);
  }
tippy('[title]', {
    placement: 'right',
    animation: 'scale',
